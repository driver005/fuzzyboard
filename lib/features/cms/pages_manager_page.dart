...
class _PagesManagerPageState extends State<PagesManagerPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          AppButton(label: 'New Page', icon: const Icon(Icons.add), size: AppButtonSize.sm, onPressed: () => _showPageDialog(context)),
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return Padding(
                      ...
                      child: AppCard(
                        leading: Container(
                          width: 36, height: 36,
                          decoration: BoxDecoration(color: _statusColor(page.status).withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
                          child: Icon(Icons.web_outlined, color: _statusColor(page.status), size: 18),
                        ),
                        title: page.title,
                        ...
                        actions: [
                          ...
                          PopupMenuButton<String>(
                            ...
                            onSelected: (action) {
                              if (action == 'edit') _showPageDialog(context, page);
                        ...
                        onTap: () => _showPageDialog(context, page),
  ...
  Color _statusColor(CmsPageStatus s) => switch (s) {
  ...
  void _showPageDialog(BuildContext context, [CmsPage? page]) {
...
class _PageStatusChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color) = switch (status) {
      CmsPageStatus.published => ('Published', const Color(0xFF10B981)),
      CmsPageStatus.draft => ('Draft', const Color(0xFFF59E0B)),
      CmsPageStatus.scheduled => ('Scheduled', const Color(0xFF3B82F6)),
    };
    return Container(
      ...
      child: ConstrainedBox(
        ...
        child: Padding(
          ...
          child: Column(children: [
            Text(widget.existing != null ? 'Edit Page' : 'New Page', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 16),
            Expanded(
              child: SingleChildScrollView(
                child: Column(children: [
                  AppInput(label: 'Title', controller: titleController, hint: 'Page title'),
                  const SizedBox(height: 12),
                  AppInput(label: 'Slug', controller: slugController, hint: '/my-page'),
                  const SizedBox(height: 12),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Template', style: Theme.of(context).textTheme.labelMedium?.copyWith(fontWeight: FontWeight.w600)),
                  ...
                  AppInput(label: 'SEO Title', controller: seoTitleController, hint: 'Browser tab title'),
                  const SizedBox(height: 12),
                  AppInput(label: 'SEO Description', controller: seoDescController, hint: 'Meta description', maxLines: 2),
                ]),
              ),
            ),
            const SizedBox(height: 16),
